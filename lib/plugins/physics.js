...
const {Physics, PlayerState, FastWorld} = require('prismarine-physics')
...
function inject(bot, {physicsEnabled, maxCatchupTicks}) {
    ...
    function updatePosition() {
        ...
        // in fact, position update should only occur if the squared distance is strictly greater than 9.0E-4D (d0 * d0 + d1 * d1 + d2 * d2 > 9.0E-4D)
        // can change this if it's a problem
        // https://github.com/Marcelektro/MCP-919/blob/1717f75902c6184a1ed1bfcd7880404aab4da503/src/minecraft/net/minecraft/client/entity/EntityPlayerSP.java#L230C29-L230C66
        ...
        const positionUpdated = dx * dx + dy * dy + dz * dz > MIN_DELTA_DIST_SQUARED ||
            ...
            // should be
            lastSent.ticker === 0
        const lookUpdated = lastSent.yaw.valueOf() !== yaw.valueOf() || lastSent.pitch.valueOf() !== pitch.valueOf()
    ...
    bot._client.on('player_rotation', (packet) => {
        setAngleDegrees(packet.yaw, packet.pitch, false)
    })
