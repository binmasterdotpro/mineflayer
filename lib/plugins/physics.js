...
const rawSensitivity = Math.random() * 0.1 + 0.05 // 0.15 is 100% sensitivity in vanilla
...
function inject(bot, {physicsEnabled, maxCatchupTicks}) {
  ...
  function updatePosition(now) {
    ...
    const positionUpdated = lastSent.x !== position.x || lastSent.y !== position.y || lastSent.z !== position.z ||
      ...
      // should be 
      (Math.round((now - lastSent.time) / PHYSICS_INTERVAL_MS) * PHYSICS_INTERVAL_MS) > 50 * (bot.version === "1.21.5" ? 19 : 20)
